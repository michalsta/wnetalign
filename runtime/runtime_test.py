...
def expand_spectrum(spectrum: Spectrum, n_times: int) -> Spectrum:
    """
    Expand a Spectrum object by repeating it a specified number of times.
    The copies are repositioned n m/z axis to avoid overlap.
    """
    positions = [spectrum.positions.copy()]
    intensities = [spectrum.intensities.copy()]
    for i in range(n_times):
        shifted_positions = spectrum.positions.copy()
        shifted_positions[0, :] += i * (np.max(spectrum.positions[0, :]) + 1)  # Shift m/z values to avoid overlap)
        positions.append(shifted_positions)
        intensities.append(spectrum.intensities.copy())
    return Spectrum(np.hstack(positions), np.hstack(intensities))
...
def align_spectra(S1: Spectrum,
    ...
    print(f"{len(S1)+len(S2)},{elapsed:.2f},{ram}")
...
def test_align_spectra(n):
    ...
    S = align_spectra(expand_spectrum(S1, n), expand_spectrum(S2, n), max_mz_shift, max_rt_shift)
    assert len(S) > 25000


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Test the alignment of spectra with increasing size.')
