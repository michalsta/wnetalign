"""Build-time provider that decides wnetalign's runtime dependencies.

``nanobind-backend`` is needed only by a *split-mode* build of ``wnetalign_cpp``.
...
BASE_DEPENDENCIES = ["pylmcf>=1.2.1", "wnet>=1.3.0", "numpy"]
