...
The pinned real-data baselines still run on exactly one canonical lane
(`linux-amd64` / default / py3.12): those numbers come out of floating-point
arithmetic feeding an exact integer solver, so a last-ulp difference between
architectures or libm versions can tip a tie between two equally-optimal
matchings. `ci_matrix.py` marks that lane with `realdata: true` and **asserts
there is exactly one of them**, in every tier; the workflow switches on
`matrix.realdata`. Everywhere else only the portable property tests run.
